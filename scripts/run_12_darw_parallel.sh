...
for s in 0 1 2; do
  LOG_DIR="${RESULTS_DIR}/grid25_darw_b1.0_s${s}/seed_${s}"
  mkdir -p "$LOG_DIR"
  echo "  GPU0: grid25 darw b1.0 seed=$s -> $LOG_DIR/train.log"
...
for s in 0 1 2; do
  LOG_DIR="${RESULTS_DIR}/mw5_darw_b0.5_s${s}/seed_${s}"
  mkdir -p "$LOG_DIR"
  echo "  GPU0: mw5 darw b0.5 seed=$s -> $LOG_DIR/train.log"
  CUDA_VISIBLE_DEVICES=0 python train.py \
...
for s in 0 1 2; do
  LOG_DIR="${RESULTS_DIR}/mw5_darw_b0.7_s${s}/seed_${s}"
  mkdir -p "$LOG_DIR"
  echo "  GPU1: mw5 darw b0.7 seed=$s -> $LOG_DIR/train.log"
...
for s in 0 1 2; do
  LOG_DIR="${RESULTS_DIR}/mw5_darw_b1.0_s${s}/seed_${s}"
  mkdir -p "$LOG_DIR"
  echo "  GPU1: mw5 darw b1.0 seed=$s -> $LOG_DIR/train.log"
  CUDA_VISIBLE_DEVICES=1 python train.py \
    experiment=mw5_darw_asbs \
    darw_beta=1.0 ksd_lambda=1.0 clip_grad_norm=1.0 \
