#!/bin/bash
# Launch all 9 SDR ASBS Grid25 experiments (3 betas x 3 seeds)
...
while [ $i -lt ${#PAIRS[@]} ]; do
  read b0 s0 <<< "${PAIRS[$i]}"
  LOG_DIR0="${RESULTS_DIR}/grid25_sdr_b${b0}_s${s0}/seed_${s0}"
  mkdir -p "$LOG_DIR0"
  echo "[$(date -u -d '+9 hours' '+%Y-%m-%d %H:%M:%S KST')] Launching beta=$b0 seed=$s0 on GPU 0"
  CUDA_VISIBLE_DEVICES=0 python train.py \
    experiment=grid25_sdr_asbs \
    sdr_beta=$b0 sdr_lambda=1.0 clip_grad_norm=1.0 \
    exp_name=grid25_sdr_b${b0}_s${s0} seed=$s0 \
  ...
  if [ $j -lt ${#PAIRS[@]} ]; then
    read b1 s1 <<< "${PAIRS[$j]}"
    LOG_DIR1="${RESULTS_DIR}/grid25_sdr_b${b1}_s${s1}/seed_${s1}"
    mkdir -p "$LOG_DIR1"
    echo "[$(date -u -d '+9 hours' '+%Y-%m-%d %H:%M:%S KST')] Launching beta=$b1 seed=$s1 on GPU 1"
    CUDA_VISIBLE_DEVICES=1 python train.py \
      experiment=grid25_sdr_asbs \
      sdr_beta=$b1 sdr_lambda=1.0 clip_grad_norm=1.0 \
      exp_name=grid25_sdr_b${b1}_s${s1} seed=$s1 \
...
echo "[$(date -u -d '+9 hours' '+%Y-%m-%d %H:%M:%S KST')] All 9 SDR Grid25 experiments finished!"
