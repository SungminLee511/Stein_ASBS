...
def main(cfg: DictConfig) -> None:
    ...
    # Periodic checkpoint saving (default: same as eval_freq)
    save_freq = getattr(cfg, 'save_freq', None) or cfg.eval_freq
    ...
    for step_idx in range(cfg.steps):
        ######### eval
        gflownet.eval()
        if not skip_eval and (step_idx % cfg.eval_freq == 0 or step_idx >= cfg.steps-1):
            ...
            if true_logz is not None:
                ...
                if logz_diff < metric_best:
                    ...
                    gflownet.save("checkpoints/checkpoint_best.pt")
                    print(f"  -> saved checkpoints/checkpoint_best.pt")
        ...
        if save_freq > 0 and step_idx > 0 and step_idx % save_freq == 0:
            ckpt_path = f"checkpoints/checkpoint_{step_idx}.pt"
            gflownet.save(ckpt_path)
            print(f"Periodic save @ step {step_idx} -> {ckpt_path}")

    # Save final checkpoint
    gflownet.save("checkpoints/checkpoint_latest.pt")
    print(f"Training done. Saved checkpoints/checkpoint_latest.pt. Best metric: {metric_best:.4f}")
