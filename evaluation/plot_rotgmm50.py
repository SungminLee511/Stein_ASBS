"""
plot_rotgmm50.py — Generate RotGMM d=50 visualization figures.
1. Mode occupation bar chart
2. PCA 2D scatter plot
Uses nearest-mode assignment (no threshold) for 50D.
"""
...
BASELINE_DIR = Path("/home/RESEARCH/Stein_ASBS/results/rotgmm50_asbs/seed_0")
KSD_DIR = Path("/home/RESEARCH/Stein_ASBS/results/rotgmm50_ksd_asbs/seed_0")
...
# ── Assign samples to nearest mode (no threshold for high-D) ──
...
ax.bar(mode_indices - bar_width, ref_frac, bar_width,
       label="Reference", color="#AAAAAA", edgecolor="black", linewidth=0.5)
ax.bar(mode_indices, bl_frac, bar_width,
       label="Baseline ASBS", color="#4C72B0", edgecolor="black", linewidth=0.5)
ax.bar(mode_indices + bar_width, ksd_frac, bar_width,
       label="KSD-ASBS", color="#DD8452", edgecolor="black", linewidth=0.5)

ax.set_xlabel("Mode Index", fontsize=13)
ax.set_ylabel("Fraction of Samples", fontsize=13)
ax.set_title("RotGMM d=50: Mode Occupation (seed=0, N=2000)", fontsize=14, fontweight="bold")
...
plt.tight_layout()
fig.savefig(FIG_DIR / "rotgmm50_mode_occupation.png", dpi=150, bbox_inches="tight")
print(f"  Saved: {FIG_DIR / 'rotgmm50_mode_occupation.png'}")
plt.close()
...
for ax, data_2d, assignments, title in [
    ...
    (axes[2], ksd_2d, ksd_assign, "KSD-ASBS"),
]:
...
fig.suptitle("RotGMM d=50: PCA Projection (seed=0, N=2000)", fontsize=15, fontweight="bold", y=1.02)
plt.tight_layout()
fig.savefig(FIG_DIR / "rotgmm50_pca_scatter.png", dpi=150, bbox_inches="tight")
print(f"  Saved: {FIG_DIR / 'rotgmm50_pca_scatter.png'}")
plt.close()

# ── Summary ──
print("\n=== Summary ===")
print(f"Baseline modes covered: {bl_covered}/8")
print(f"KSD-ASBS modes covered: {ksd_covered}/8")
print(f"Baseline per-mode counts: {bl_counts.tolist()}")
print(f"KSD-ASBS per-mode counts: {ksd_counts.tolist()}")
print("\nDone!")
