"""
eval_rotgmm30_v2.py — Evaluate RotGMM d=30 with dimension-corrected mode coverage.

In D dimensions, a sample from N(μ, σ²I) has expected L2 distance σ√D from μ.
For d=30, σ=0.5: expected dist = 0.5*√30 ≈ 2.74.
The fixed threshold of 3σ = 1.5 misses everything.

We use threshold = threshold_factor * σ * √D instead.
"""
...
def count_modes_dim_corrected(samples, energy, threshold_factor=3.0):
    """Count modes covered with dimension-corrected threshold.

    threshold = threshold_factor * mode_std * sqrt(dim)

    For d=30, σ=0.5, factor=3: threshold = 3 * 0.5 * √30 ≈ 8.22
    This captures ~99.7% of probability mass of each mode.
    """
    centers = energy.get_mode_centers().to(samples.device)
    dim = samples.shape[1]
    mode_std = energy.mode_std
    threshold = threshold_factor * mode_std * np.sqrt(dim)
    ...
    per_mode_counts = (dists < threshold).sum(dim=0)  # (K,)

    # Also use nearest-mode assignment (no threshold)
...
def compute_metrics(samples, energy, ref_samples=None):
    ...
    # Mode coverage (dimension-corrected)
    cov = count_modes_dim_corrected(samples, energy, threshold_factor=3.0)
    ...
    metrics['threshold_used'] = cov['threshold_used']
...
def main():
    ...
    for exp_name, run_dir in EXPERIMENTS.items():
        ...
        for eval_seed in range(N_EVAL_SEEDS):
            ...
            print(f"    energy_w2={m.get('energy_w2', 'N/A'):.4f}, "
                  ...
                  f"modes_covered={m['n_modes_covered']}/{m['n_modes_total']} "
                  f"(nearest={m['n_modes_nearest']})")
            print(f"    nearest_counts={m['nearest_mode_counts']}")
            print(f"    min_dists={[f'{d:.2f}' for d in m['min_dists_to_centers']]}")
        ...
        for key in seed_metrics[0]:
            ...
            if isinstance(vals[0], (int, float)):
                ...
                # Aggregate list metrics: mean across seeds
                arr = np.array(vals)  # (n_seeds, n_modes)
                agg[key] = {
        ...
        for k in ['mean_energy', 'energy_w2', 'ksd_squared', 'n_modes_covered',
                   'coverage_fraction', 'n_modes_nearest']:
            if k in agg:
    ...
    for metric in ['mean_energy', 'energy_w2', 'ksd_squared', 'n_modes_covered',
                   'coverage_fraction', 'n_modes_nearest']:
    ...
    # Nearest-mode counts (average across seeds)
