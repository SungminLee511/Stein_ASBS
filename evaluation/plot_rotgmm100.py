"""
plot_rotgmm100.py — Generate RotGMM d=100 visualization figures.
Nearest-mode assignment (no threshold for 100D).
"""
...
BASELINE_DIR = Path("/home/RESEARCH/Stein_ASBS/results/rotgmm100_asbs/seed_0")
KSD_DIR = Path("/home/RESEARCH/Stein_ASBS/results/rotgmm100_ksd_asbs/seed_0")
...
ax.bar(mode_indices + bar_width, ksd_frac, bar_width,
       label="KSD-ASBS (λ=0.1)", color="#DD8452", edgecolor="black", linewidth=0.5)

ax.set_xlabel("Mode Index", fontsize=13)
ax.set_ylabel("Fraction of Samples", fontsize=13)
ax.set_title("RotGMM d=100: Mode Occupation (seed=0, N=2000)", fontsize=14, fontweight="bold")
...
plt.tight_layout()
fig.savefig(FIG_DIR / "rotgmm100_mode_occupation.png", dpi=150, bbox_inches="tight")
print(f"  Saved: {FIG_DIR / 'rotgmm100_mode_occupation.png'}")
plt.close()
...
for ax, data_2d, assignments, title in [
    ...
    (axes[2], ksd_2d, ksd_assign, "KSD-ASBS (λ=0.1)"),
]:
...
fig.suptitle("RotGMM d=100: PCA Projection (seed=0, N=2000)", fontsize=15, fontweight="bold", y=1.02)
plt.tight_layout()
fig.savefig(FIG_DIR / "rotgmm100_pca_scatter.png", dpi=150, bbox_inches="tight")
print(f"  Saved: {FIG_DIR / 'rotgmm100_pca_scatter.png'}")
plt.close()
